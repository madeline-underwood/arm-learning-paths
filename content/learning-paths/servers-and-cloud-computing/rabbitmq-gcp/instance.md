...
You can now provision a Google Axion C4A Arm virtual machine on Google Cloud Platform (GCP) using the `c4a-standard-4` (4 vCPUs, 16 GB memory) machine type in the Google Cloud Console.  
...
Under **Machine configuration**, populate fields such as **Instance name**, **Region**, and **Zone**. Set **Series** to `C4A` and select `c4a-standard-4` for machine type.
   ...
   ![Google Cloud Console showing C4A machine configuration with c4a-standard-4 selected alt-text#center](images/gcp-vm.png "Creating a Google Axion C4A Arm virtual machine in Google Cloud Console")
...
![Google Cloud Console displaying network settings with allow-tcp-15672 network tag alt-text#center](images/network-config.png "Adding the TCP/15672 firewall rule to the VM")
...
![Google Cloud Console VM instances list showing SSH button and public IP address alt-text#center](images/gcp-pubip-ssh.png "Invoke an SSH session into your running VM instance")

A window opens from your browser and you see a shell into your VM instance.

![Terminal window showing an active SSH session into the VM alt-text#center](images/gcp-shell.png "Terminal shell in your VM instance")
